...
module Idl
    ...
    def value(symtab)
      ...
      value =
        if op == ">>>"
          ...
          # convert to unsigned if needed
          v = v & ((1 << type(symtab).width) - 1) if v < 0 && !type(symtab).signed?
          ...
          if v != v_trunc
            warn "At #{input_file}:#{lineno}"
            warn "WARNING: The value of '#{text_value}' is truncated from #{v} to #{v_trunc} because the result is only #{type(symtab).width} bits"
          end
          v_trunc
        end
    ...
    def type_check(symtab)
      ...
      if obj_type.kind == :bitfield
        ...
        unless obj_type.field_names.include?(@field_name)
          type_error "#{@field_name} is not a member of #{obj_type} (members: #{obj_type.field_names})"
        end
      elsif obj_type.kind == :struct
  ...
  class UnknownLiteral
    attr_reader :known_value
    attr_reader :unknown_mask
    def initialize(known_value, unknown_mask)
      @known_value = known_value
      @unknown_mask = unknown_mask
    end

    def bit_length = @known_value.bit_length
  end
